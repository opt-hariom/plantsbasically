...
(function(){
  ...
  theme.Product = (function() {
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      updatePrice: function(evt) {
        ...
        if (variant) {
          // Regular price
          console.log(variant.price,this.cache.price);
          ...
          setTimeout(function(e) {
            var discount_price = $('#rc_container #rc_radio_options .rc_block__type__autodeliver .rc_price.rc_price__autodeliver').text();
            $('.subscrption_item.subscrption .subscrption_price').text(discount_price);
  
            var price = $('#rc_radio_options .rc_block__type__onetime .rc_label.rc_label__onetime .rc_price__onetime').text();
            $('.subscrption_item.one-time .subscrption_price').text(price);
          },100)
        }
      },
  ...
  $(window).on('load',function(e) {
    var discount = $('#rc_container #rc_radio_options .rc_block__type__autodeliver .rc_label__discount').text().replace('):','').replace('(','');
    var html = '';
    $('#rc_container .rc_select__frequency  option').each(function(e) {
      var val = $(this).val();
      var text = $(this).text();
    ...
    $('.subscrption_select__frequency').html(html);
    $('.subscrption_discount').text('Save '+discount);
    ...
    setTimeout(function(e) {
      clearInterval(inter);
      var len = $('#rc_container').length;
      if(len > 0){
        //$('#rc_radio_options .rc_block__type__autodeliver > label')[0].click();
        $('.subscrption_widget_custom').removeAttr('style');
      }
    },600)

    $('.subscrption_widget_custom .subscrption_item').on('click',function(e) {
       $(this).closest('.subscrption_widget_custom').find('.subscrption_item').removeClass('active');
       $(this).addClass('active'); 
       if($(this).hasClass('one-time')){
         if(!$('#rc_radio_options .rc_block__type__onetime').hasClass('rc_block__type--active')){
          $('#rc_radio_options .rc_block__type__onetime > label')[0].click();  
         }
       }else{
        if(!$('#rc_radio_options .rc_block__type__autodeliver').hasClass('rc_block__type--active')){
         $('#rc_radio_options .rc_block__type__autodeliver > label')[0].click();
        }
       }
    })
    
    $('.delivery_msg .subscrption_select__frequency').unbind('click').on('change',function(e) {
       e.preventDefault(); 
       var val = $(this).val();
       $('#rc_autodeliver_options .rc_select__frequency').val(val).trigger('change'); 
       return false; 
    })

    $(document).ready(function() {
  var perksHTML = `<div class="subscription-perks">
                     <a href="#" id="togglePerks">Your subscription perks +</a>
                     <div id="perksContent" style="display:none;">
                         <img src="path-to-your-image.jpg" alt="Subscription Perks">
                     </div>
                   </div>`;
  // Assuming .subscrption_select__frequency is inside a specific container you can access
  $('.subscrption_select__frequency').closest('div').after(perksHTML);

  // Toggle functionality
  $('#togglePerks').click(function(e) {
    e.preventDefault();
    $('#perksContent').slideToggle('fast');
  });
});
